...
Sinds versie 66 in de **sporter-app**: maaltijdsjablonen ("Mijn maaltijden": een maaltijdmoment
in één keer bewaren en later in één tik terugzetten), vaak gebruikt, kopieer gisteren, barcode
(camera of cijfers intypen), een basislijst van ±70 Nederlandse producten zonder internet, en
het weekgemiddelde naast het plan van de coach. Nog open: de sjablonen staan alleen op het
toestel van de klant zelf; echte recepten met grammen per ingrediënt zijn er niet.
...
## Versie 66 — coachlog én sporterslog verder uitgebouwd (3 sep 2026)

Vraag: "alles erbij halen wat we in de coachlog en sporterslog kunnen toevoegen". Gekozen voor
wat een klant in de sportschool en thuis het meest mist tegenover Strong/Hevy/Trainerize, plus
wat de coach dagelijks scheelt — zonder nieuwe tabellen, zodat er geen SQL-opzetwerk bij komt.

**Coach-app**
- Vandaag toont een vak **Van klanten**: ongelezen berichten en check-ins van de laatste week,
  met een knop naar de juiste klant. Leeg vak = geen nieuws.
- **Bericht aan iedereen** in Van klanten (vakantiemelding, gewijzigde openingstijden).
- Nieuwe uitnodigingscode geeft nu een **koppel-link** met project-URL, sleutel, code en naam
  erin, met kopieer- en deelknop. Past hij in een QR (korte `sb_publishable_…`-sleutel), dan
  staat die ernaast; een oude JWT-anon-key is te lang voor de eigen QR-encoder (versie 9).
- Klaarzetten stuurt per oefening de **uitleg, aandachtspunten, videolink en technieknotitie**
  mee, zodat de klant ze tijdens de training ziet.
- Overzicht → Spiergroepen rekent **werksets per trainingsweek** uit en zet ze tegen de
  richtlijn 10–20; te weinig of te veel wordt benoemd.
- Bugfix: de Overzicht-pagina toonde een verdwaald "Uitleg"-vak (via `window.name`) — weg.

**Sporter-app**
- Een begonnen training **overleeft het sluiten** van de app; weggooien kan expliciet.
- Sessieklok, en **leeg afvinken neemt de vorige keer over** (de spookwaarden worden echt).
- **Cardio** logt minuten en kilometers, **lichaamsgewicht** extra kilo's en herhalingen; de
  soort komt uit het schema of is te kiezen bij een losse oefening.
- ⓘ toont de uitleg van de coach; **+ Warming-up** zet 50/70/85%-opbouwsets klaar (tellen niet
  mee voor records); **schijven per kant** staan live onder de sets.
- **Records** bij opslaan (vergeleken met alles wat eerder gelogd is), trilling en melding.
- Voortgang: **geschat 1RM** (Epley), aanwezigheid van twaalf weken, **geschiedenis** met
  **Herhaal** (vorige waarden als doel), en "Vorige training herhalen" op het startscherm.
- Training **delen** als tekst (deelvenster of klembord).
- Voeding: barcode, basislijst zonder internet, vaak gebruikt, kopieer gisteren, Mijn maaltijden,
  weekgemiddelde tegen het plan, instelbaar waterdoel.
- **Instellingen** onderaan Coach: rusttijd (als het schema er geen meegeeft), geluid, licht of
  donker thema, waterdoel.
- De koppel-link (`?url=&key=&code=&naam=`) vult de app in één keer in; alleen inloggen blijft.

Getest met Playwright tegen een nagebootste Supabase (page.route), beide apps zonder JS-fouten.
Niet gebouwd, bewust: pushmeldingen/mail aanzetten (opzetwerk, zie hierboven), meerdere coaches,
en synchroniseren van Mijn maaltijden tussen toestellen (vraagt een tabel).
